...
str(combi)
nums <- sapply(pay_tr, is.numeric)
nums_data= pay_tr[,nums]
str(nums_data)
...
corr.idx <- names(which(apply(corr.SalePrice, 1, function(x) (x > 0.03 | x < -0.03))))
...
ggplot(pay_tr,aes(x=log(Pay_in_INR)))+
  ...
  labs(y="Count",
       title="Distribution of pay")
aa= data.frame("log$Pay_in_INR"= log(pay_tr$Pay_in_INR))
summary(aa)
...
table(combi$Gender)
# Very few females 
ggplot(pay_tr,aes(y=Pay_in_INR,x=Gender))+
...
ggplot(pay_tr,aes(fill=Gender,x=log(Pay_in_INR)))+
...
pay_tr$CollegeTier= as.factor(pay_tr$CollegeTier)
ggplot(pay_tr,aes(y=Pay_in_INR,x=CollegeTier))+
...
# Right! It does intuitively seem that tier 1 students get more extreme pays. This is a 
# strong predictor 
...
cor(pay_tr$Pay_in_INR, pay_tr$GPA.Score.in.Graduation)
...
ggplot(pay_tr,aes(y=Pay_in_INR,x=State))+
...
combi2= rbind(pay_tr, pay_te)
sum(is.na(combi$Year.of.Graduation.Completion))
# No NAs 
str(combi)
table(combi$Year.of.Graduation.Completion)
ggplot(combi2,aes(y=Pay_in_INR,x= as.factor(Year.of.Graduation.Completion)))+
...
print(DT_rpart_Reg)
class(DT_rpart_Reg)
class(ntrain$Pay_in_INR)
dt= rpart (Pay_in_INR~. , data=ntrain, control=rpart.control(cp= 0.0001))
fancyRpartPlot(dt)
...
error
predd= predict(DT_rpart_Reg)
error2= regr.eval(predd, ntrain$Pay_in_INR)
error2
...
dim(ntrain)
?randomForest
model_rf = randomForest(Pay_in_INR ~ . , ntrain, ntree = 100,mtry = 6)
summary(model_rf)
plot(model_rf)
varImpPlot(model_rf)
par(mfrow=c(1,1))
